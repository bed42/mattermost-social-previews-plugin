...
import (
	...
	"github.com/bed/mastodon-mattermost-plugin/server/command"
	"github.com/bed/mastodon-mattermost-plugin/server/store/kvstore"
...
func (p *Plugin) MessageWillBePosted(c *plugin.Context, post *model.Post) (*model.Post, string) {
	...
	for _, url := range mastodonURLs {
		...
		if err != nil {
			p.API.LogWarn("Failed to fetch Mastodon post", "url", url, "error", err.Error())
			continue
		}
	...
	if len(attachments) > 0 {
		...
		existingAttachments, ok := post.Props["attachments"].([]interface{})
		if ok {
